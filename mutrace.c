...
static bool mutex_info_dump(struct mutex_info *mi) {
        ...
        fprintf(stderr,
                "\nMutex #%u (%p) first referenced by:\n"
                "%s", mi->id,
#if HAVE_ISC_RWLOCK_H
                mi->mutex ? (void*) mi->mutex : mi->rwlock ? (void*) mi->rwlock : (void*) mi->rwl,
#else
                mi->mutex ? (void*) mi->mutex : (void*) mi->rwlock,
#endif
                stacktrace_to_string(&(mi->origin_stacktrace)));
...
static bool cond_info_dump(struct cond_info *ci) {
        ...
        fprintf(stderr,
                "\nCondvar #%u (%p) first referenced by:\n"
